...
for step in timeline:
    ...
    for decision in decisions:
        ...
        if old_action != new_action:
            ...
            # The first assignment is initialization, not migration.
            # previous_actions starts with a default onboard/full state, so
            # step 0 must be classified explicitly as Initial Placement.
            if step.time == 0:
                transition_type = "Initial Placement"
            else:
                transition_type = decision["adaptation_type"]
