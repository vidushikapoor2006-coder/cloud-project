...
def calculate_resource_aware_score(
    ...
    resource = {
        ...
        "Edge_1": system.edge_1,
        "Edge_2": system.edge_2,
        "AWS_Cloud": system.cloud,
    }[location.value]

    resource_normalized = (
        workload.compute_requirement
        / max(resource.cpu_capacity, 1)
    )
...
def select_resource_aware_action(
    ...
    # Preserve the highest feasible service level first.
    # Within that level, minimize resource/energy/latency cost.
    service_rank = {
        ExecutionMode.FULL: 3,
        ExecutionMode.REDUCED: 2,
        ExecutionMode.FALLBACK: 1,
        ExecutionMode.SUSPENDED: 0,
    }

    best_service_rank = max(
        service_rank[candidate[2]]
        for candidate in candidates
        if candidate[2] != ExecutionMode.SUSPENDED
    ) if any(candidate[2] != ExecutionMode.SUSPENDED for candidate in candidates) else None

    if best_service_rank is not None:
        service_candidates = [
            candidate for candidate in candidates
            if candidate[2] != ExecutionMode.SUSPENDED
            and service_rank[candidate[2]] == best_service_rank
        ]
